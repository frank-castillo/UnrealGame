...
bool FWwiseReconcile::IsAssetOutOfDate(FAssetData AssetData, const FWwiseAnyRef& WwiseRef)
{
	...
	if (StringGuid.IsSet())
	{
		return WwiseRef.GetGuid() != FGuid(StringGuid.AsString()) ||
			ShortId != WwiseRef.GetId();
	}
	return false;
}
...
TArray<FAssetData> FWwiseReconcile::CreateAssets(FScopedSlowTask& SlowTask)
{
	...
	for (const auto& Asset : AssetsToCreate)
	{
		...
		FString AssetPath = AssetPackagePath / AssetName.ToString();
		int MaxPath = 0;
		//Windows Max Path Length can be 32767 while the max path length for cooking remains 260.
#if PLATFORM_WINDOWS
		MaxPath = MAX_PATH;
		...
		MaxPath = FPlatformMisc::GetMaxPathLength();
#endif
		if (AssetPath.Len() > MaxPath || AssetPath.Len() >= NAME_SIZE)
		{
